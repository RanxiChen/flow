"""Small 16550-compatible register front-end backed by LiteX stream FIFOs."""

from migen import Array, Cat, If, Module, Mux, Signal
from litex.soc.interconnect import stream, wishbone


class BreezeUart16550(Module):
    def __init__(self, fifo_depth=16):
        if fifo_depth < 1:
            raise ValueError("16550 FIFOs require a positive depth")
        self.bus = bus = wishbone.Interface(
            data_width=64, address_width=32, addressing="word")
        self.tx = stream.Endpoint([("data", 8)])
        self.rx = stream.Endpoint([("data", 8)])
        self.interrupt = Signal()

        # Use the same buffered FIFO structure as LiteX's UART core.  In
        # particular, architectural UART status must describe the local
        # holding/FIFO state, not the downstream PHY's combinational ready.
        # Some simulation backends assert ready only after valid; tying THRE
        # directly to ready creates a valid/ready deadlock before the first
        # console byte.
        self.submodules.tx_fifo = tx_fifo = stream.SyncFIFO(
            [("data", 8)], fifo_depth, buffered=True)
        self.submodules.rx_fifo = rx_fifo = stream.SyncFIFO(
            [("data", 8)], fifo_depth, buffered=True)
        self.comb += [
            tx_fifo.source.connect(self.tx),
            self.rx.connect(rx_fifo.sink),
        ]

        ier = Signal(4)
        lcr = Signal(8, reset=0x03)
        mcr = Signal(8)
        scr = Signal(8)
        dll = Signal(8, reset=1)
        dlm = Signal(8)
        dlab = lcr[7]
        lane = Signal(3)
        lane_found = Signal()
        # Lowest selected byte lane is the addressed 16550 byte register.
        self.comb += [lane.eq(0), lane_found.eq(0)]
        for n in reversed(range(8)):
            self.comb += If(bus.sel[n], lane.eq(n), lane_found.eq(1))
        reg_offset = Signal(3)
        self.comb += reg_offset.eq(lane)
        write_bytes = Array(bus.dat_w[8*n:8*(n+1)] for n in range(8))
        write_data = write_bytes[lane]

        request = bus.cyc & bus.stb & lane_found
        tx_access = request & bus.we & (reg_offset == 0) & ~dlab
        rx_access = request & ~bus.we & (reg_offset == 0) & ~dlab
        can_respond = Mux(tx_access, tx_fifo.sink.ready, 1)
        responding = Signal(reset=0)
        read_byte = Signal(8)
        read_word = Signal(64)
        tx_empty = Signal()
        rx_valid = rx_fifo.source.valid
        self.comb += [
            tx_empty.eq(~tx_fifo.source.valid),
            tx_fifo.sink.valid.eq(tx_access & ~responding),
            tx_fifo.sink.data.eq(write_data),
            rx_fifo.source.ready.eq(rx_access & ~responding),
            bus.ack.eq(responding & bus.cyc & bus.stb),
            bus.err.eq(0),
            read_byte.eq(0),
            read_word.eq(read_byte << (lane << 3)),
            bus.dat_r.eq(read_word),
            self.interrupt.eq((ier[0] & rx_valid) | (ier[1] & tx_empty)),
        ]
        self.comb += If(reg_offset == 0,
                        read_byte.eq(Mux(dlab, dll,
                            Mux(rx_valid, rx_fifo.source.data, 0)))) \
            .Elif(reg_offset == 1, read_byte.eq(Mux(dlab, dlm, ier))) \
            .Elif(reg_offset == 2,
                  # IIR[7:6]=11 advertises the buffered 16550A FIFOs.
                  read_byte.eq(0xC0 | Mux(ier[0] & rx_valid, 0x04,
                                      Mux(ier[1] & tx_empty, 0x02, 0x01)))) \
            .Elif(reg_offset == 3, read_byte.eq(lcr)) \
            .Elif(reg_offset == 4, read_byte.eq(mcr)) \
            .Elif(reg_offset == 5,
                  # DR, THRE and TEMT reflect local FIFO state.  Both TX bits
                  # are set after reset, even if the PHY has not asserted
                  # ready yet.
                  read_byte.eq(Mux(rx_valid, 0x01, 0x00) |
                               Mux(tx_empty, 0x60, 0x00))) \
            .Elif(reg_offset == 6,
                  # CTS, DSR and DCD asserted: no external modem model.
                  read_byte.eq(0xB0)) \
            .Elif(reg_offset == 7, read_byte.eq(scr))

        self.sync += [
            If(request & ~responding & can_respond,
               responding.eq(1),
               If(bus.we,
                  If(reg_offset == 0,
                     If(dlab, dll.eq(write_data)))
                  .Elif(reg_offset == 1,
                        If(dlab, dlm.eq(write_data)).Else(ier.eq(write_data[:4])))
                  # FCR (offset 2) is deliberately accepted as a no-op.
                  .Elif(reg_offset == 3, lcr.eq(write_data))
                  .Elif(reg_offset == 4, mcr.eq(write_data))
                  .Elif(reg_offset == 7, scr.eq(write_data))))
            .Elif(~bus.cyc | ~bus.stb, responding.eq(0))
        ]
