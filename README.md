...
实现切换为独立的 `FlowPlic.sv`，LiteX 只负责实例化和总线接线；旧实现仍保留给 MCU
回归和对照。目前这条新 PLIC 路径只完成静态检查，短测试及后续 handoff、Linux kernel
与用户空间仍需要目标机 Verilator 运行证据。不能把“镜像成功装入 DDR”或“Python
单元测试通过”当成“Linux 已启动”。详细证据见
