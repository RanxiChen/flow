...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--nm",
        default="riscv64-unknown-elf-nm",
        help="nm executable used to locate the ACT tohost symbol.",
    )
    parser.add_argument(
        "--max-cycles",
        default="1000000",
        help="Per-test simulation cycle limit.",
    )
...
def find_symbol(elf_path: Path, nm_executable: str, symbol: str) -> int:
    result = subprocess.run(
        [nm_executable, "-P", str(elf_path)],
        check=True,
        capture_output=True,
        text=True,
    )
    ...
    raise ValueError(f"{elf_path} does not define required symbol {symbol}")
    ...
    *,
    ...
    return {
        ...
        "simulation": {
            ...
            "exitAddress": f"0x{exit_address:x}",
...
def main() -> None:
    ...
    exit_address = find_symbol(elf_path, args.nm, "tohost")
    max_cycles = parse_numeric(args.max_cycles)
    if max_cycles <= 0:
