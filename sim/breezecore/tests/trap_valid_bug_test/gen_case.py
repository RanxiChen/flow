    ...
    for index in range(e_phnum):
        ...
        if elf_class == 1:
            (p_type, p_offset, p_vaddr, p_paddr, p_filesz, p_memsz,
             _p_flags, _p_align) = struct.unpack_from(ph_fmt, raw, ph_off)
        else:
            (p_type, _p_flags, p_offset, p_vaddr, p_paddr, p_filesz,
             p_memsz, _p_align) = struct.unpack_from(ph_fmt, raw, ph_off)
...
def build_output_document(
    *, test_name: str, source_path: Path, elf_path: Path,
    output_path: Path, bootaddr: int, tandem_log: bool,
    memory_map: dict[str, str],
) -> dict[str, object]:
    return {
        "meta": {
...
def main() -> None:
    ...
    doc = build_output_document(
        test_name=elf_path.stem, source_path=source_path,
        elf_path=elf_path, output_path=output_path,
        bootaddr=bootaddr, tandem_log=tandem_log, memory_map=memory_map,
    )
