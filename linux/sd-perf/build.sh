...
out=${BUILDROOT_OUT:-$repo/build/sd-perf/output}
boot=${SD_PERF_BOOT:-$repo/build/sd-perf/boot}
mkdir -p "$out" "$boot"
make -C "$BUILDROOT_DIR" O="$out" BR2_EXTERNAL="$repo/linux/buildroot-external" flow_tiny_perf_defconfig
make -C "$BUILDROOT_DIR" O="$out" -j"${JOBS:-8}"
for name in Image fw_jump.bin flow-kcu105-tiny-perf.dtb; do
...
boot={'Image':'0x80200000','flow-kcu105-tiny-perf.dtb':'0x80100000','fw_jump.bin':'0x80000000','serial-handoff.bin':'0x80080000','addr':'0x80080000'}
