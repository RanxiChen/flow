...
/**
  ...
  * 为了实现konata，需要维护一个retired 计数器，来记录已经retire的指令数量
  */
class VirFlow(val dumplog: Boolean = false) extends Module {
    val io = IO(new Bundle {
        val reset_addr = Input(UInt(64.W))
        val mem = new NativeMemIO
    })
    val fe = Module(new GustEngine(dumplog))
    val instbuf = Module(new InstBuffer(dumplog = dumplog))
    val be = Module(new Backend(dumplog))
