...
class BreezeBackendEXEMEM(val VLEN: Int = 64, val ghrLength: Int = 0, val enableTandem: Boolean = false) extends Bundle {
    ...
    val mul_b = SInt(65.W)
    val mul_valid = Bool()
    val mul_op = UInt(MUL_OP.width.W)
    val div_valid = Bool()
    val div_fast = Bool()
    val div_fast_result = UInt(VLEN.W)
    val div_dividend_mag = UInt(VLEN.W)
    val div_divisor_mag = UInt(VLEN.W)
    val div_quotient_neg = Bool()
    val div_remainder_neg = Bool()
    val div_is_remainder = Bool()
    val div_is_word = Bool()
...
class FASECoreIO() extends Bundle {
    ...
    val inst_pc = Input(UInt(64.W))
    val halt = Input(Bool())
    val launch = Input(Bool())
    val launchPc = Input(UInt(64.W))
    ...
    val regWdata = Input(UInt(64.W))
    val regRdata = Output(UInt(64.W))
    val diagnostic = Output(Vec(48, UInt(64.W)))
}

/** Optional backend port. Absent in production elaborations. */
class FaseBackendIO extends Bundle {
